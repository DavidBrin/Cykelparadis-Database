...
// Query 3
SELECT part_code, manufacturer_name, repair_job_parts.qty as total_quantity
FROM parts
NATURAL LEFT JOIN repair_job_parts
JOIN repair_jobs
	ON repair_jobs.parts_list_id = repair_job_parts.listid
WHERE YEAR(repair_jobs.job_date) = 2024
...
SELECT bike_type, bike_id, manufacturer_name FROM
(
    SELECT bikes.bike_type, bikes.bike_id, bikes.manufacturer_name, Count(bikes_repaired.bike_id) as number_of_repairs,
ROW_NUMBER() OVER (PARTITION BY bikes.bike_type ORDER BY Count(bikes_repaired.bike_id) DESC) as row_no
FROM bikes 
NATURAL LEFT JOIN repair_jobs bikes_repaired
GROUP BY bikes.bike_id, bikes.bike_type, bikes.manufacturer_name
) ranked_by_repairs
WHERE row_no = 1;
...
// Query 5
SELECT DISTINCT bikes.bike_id, bikes.manufacturer_name 
FROM bikes
WHERE bikes.bike_id NOT IN (
	SELECT compatible_parts.bike_id
